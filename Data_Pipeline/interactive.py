...
from config import (
    ...
    DATA_ROOT,
    build_lean_data_path,
    get_provider_api_status,
    get_ytd_window,
)
...
def confirm_lean_format(symbols: List[str], resolution: str) -> bool:
    """Ask the user whether to save data in the QuantConnect Lean folder layout."""
    if not Confirm.ask("Save files in Lean folder layout under data/?", default=True):
        return False

    for symbol in symbols:
        info = classify_ticker(symbol)
        lean_path = build_lean_data_path(info.asset_type, symbol, resolution)
        lean_path.mkdir(parents=True, exist_ok=True)
        console.print(f"  [green]{lean_path}[/green]")
...
def main():
    try:
        ...
        # Step 6: Ask whether to save in Lean format
        options["lean_format"] = confirm_lean_format(symbols, resolution)
